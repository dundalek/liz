...
fn fetchUrl(allocator: *std.mem.Allocator, url: []const u8) anyerror![]u8 {
    ...
    if (suspend_download) {
        suspend global_download_frame = @frame();
    }
    if (simulate_fail_download) {
...
fn readFile(allocator: *std.mem.Allocator, filename: []const u8) anyerror![]u8 {
    ...
    errdefer allocator.free(result);
    if (suspend_file) {
        suspend global_file_frame = @frame();
    }
    if (simulate_fail_file) {
